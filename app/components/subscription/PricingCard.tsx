...
// app/components/subscription/PricingCard.tsx - Fixed client-side imports
'use client';
...
import { SUBSCRIPTION_TIERS } from '@config/subscription';
import { formatPrice } from '@utils/formatters';
// Remove server-side import - use client-side function instead
// import { createCheckoutSession } from '@lib/subscription/stripe';
import type { SubscriptionTierType } from '../../../types/subscription';
...
export const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const getButtonText = () => {
    ...
    if (isDowngrade) return `Downgrade to ${tierConfig.name}`;
    return `Get ${tierConfig.name}`;
  };
