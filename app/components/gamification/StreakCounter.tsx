...
import { calculateCurrentStreak, getStreakMotivation, isStreakActive } from '@utils/helpers';
...
export const StreakCounter: React.FC<StreakCounterProps> = ({
  ...
  const streakData = user.streakData || { current: 0, longest: 0, lastStoryDate: null };
  ...
  if (compact) {
    return (
      <div className={`flex items-center space-x-3 ${className}`}>
        ...
        >
          <Flame className={`${streakColor} ${currentStreak >= 7 ? 'animate-pulse' : ''}`} size={20} />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <motion.div
              animate={animated && isActive && currentStreak > 0 ? { 
                scale: [1, 1.2, 1],
                rotate: [0, 10, -10, 0]
              } : {}}
              transition={{ 
           ...
           <div className="flex items-center space-x-3">
             ...
             <div className="flex-1">
               ...
               <p className="text-xs text-orange-700 dark:text-orange-300">
                 Write a story in the next {timeUntilReset.hours}h {timeUntilReset.minutes}m to keep your {currentStreak}-day streak alive.
               </p>
             </div>
             {onWriteStory && (
               <Button variant="warning" size="sm" onClick={onWriteStory}>
                 Write Now
               </Button>
             )}
           </div>
         ...
         <div className="space-y-3">
           ...
           <div className="flex items-center justify-center">
             <Badge variant="warning" size="sm">
               ...
               +{nextMilestone.points} points reward
             </Badge>
           </div>
         </div>
       )}
           ...
           {STREAK_MILESTONES.slice(0, 5).map((milestone, index) => {
             ...
             return (
               ...
               >
                 ...
                 <Badge
                   variant={isReached ? 'success' : isCurrent ? 'warning' : 'default'}
                   size="sm"
                 >
                   +{milestone.points}
                 </Badge>
       ...
       {onWriteStory && (
         <Button
           ...
           className="w-full"
           disabled={isActive && streakData.lastStoryDate && 
             new Date(streakData.lastStoryDate).toDateString() === new Date().toDateString()
           }
         >
           {isActive && streakData.lastStoryDate && 
            new Date(streakData.lastStoryDate).toDateString() === new Date().toDateString()
             ? '✅ Story Written Today!'
             : '🔥 Write Story Today'
           }
