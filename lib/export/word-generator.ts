...
export class WordGenerator {
  ...
  async generateStoryDocument(storyData: StoryWordData): Promise<Buffer> {
    try {
      ...
      const elementsSection = this.createElementsSection(
   ...
   */
  async generatePortfolioDocument(
    stories: StoryWordData[],
    userName: string
  ): Promise<Buffer> {
    try {
      ...
      const storySections: ISectionOptions[] = stories.map(
        (story: StoryWordData, index: number) => {
          if (!story) {
            // Return empty section for undefined stories
            return {
              properties: {},
              children: [],
            };
          ...
          return {
            ...
            children: [
              ...
              ...(story.assessment
                ? this.createAssessmentSection(story.assessment)
                : []),
      ...
      const doc = new Document({
        sections: [
          {
            properties: {},
            children: [...coverPage, ...tableOfContents],
          },
          ...storySections,
        ],
      });
  ...
  private createElementsSection(elements: StoryElements): Paragraph[] {
      ...
      rows: [
        ...
        new TableRow({
          children: [
            ...
            new TableCell({
              children: [
                new Paragraph({ text: `Setting: ${elements.setting}` }),
              ],
            }),
          ],
        }),
        new TableRow({
          children: [
            new TableCell({
              children: [
                new Paragraph({ text: `Character: ${elements.character}` }),
              ],
            }),
            new TableCell({
        ...
        new TableRow({
          children: [
            new TableCell({
              children: [
                new Paragraph({ text: `Conflict: ${elements.conflict}` }),
              ],
            }),
            new TableCell({
  ...
  private createAssessmentSection(assessment: Assessment): Paragraph[] {
    ...
    if (assessment.suggestions && assessment.suggestions.length > 0) {
      ...
      assessment.suggestions.forEach((suggestion: string) => {
        sections.push(
          new Paragraph({
            text: `• ${suggestion}`,
            spacing: {
  ...
  private createTableOfContents(stories: StoryWordData[]): Paragraph[] {
    ...
    stories.forEach((story: StoryWordData, index: number) => {
      if (!story) return; // Skip undefined stories
